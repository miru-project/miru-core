...
// TestMiruroMirrorHandler verifies that the golang (v2) Mirror path no longer
// ships a raw JSON fallback. MirrorResponse carries only typed members, and a
// golang mirror list has no typed home in that oneof, so the handler must
// return an error telling the client to resolve mirrors from the Watch
// response instead.
//
// The handler still performs extension loading/network setup without error; the
// error is the expected "does not conform" result for the golang runtime.
func TestMiruroMirrorHandler(t *testing.T) {
	...
	_, err := s.Mirror(context.Background(), &proto.MirrorRequest{
		Pkg: "miruro.org",
		Url: m3u8,
	})
	if err == nil {
		t.Fatalf("expected Mirror handler to error for golang extension (no typed mirror member), got nil")
	}
	if !strings.Contains(err.Error(), "does not conform") {
		t.Fatalf("expected 'does not conform' error, got: %v", err)
	}
	t.Logf("Mirror(golang) correctly errored: %v", err)
}
